            ...
            <div class="row justify-content-between align-items-end">
              <div class="col-lg-8">
                <h2 class="display-3 text-white">
                  {{ $t('node.title') }}
                  <span>{{ $t('node.span') }}</span>
                </h2>
                <p class="lead  text-white">
                  {{ $t('node.location') }} {{ data[0].address }}.
                </p>
              </div>
              <div class="col-lg-4">
...
export default {
  ...
  head () {
    return {
      title: this.$i18n.locale === 'vi' ? `Biểu đồ tại ${this.data[0].address}` : `The chart at ${this.data[0].address}`,
      meta: [
        { property: 'og:title', content: `Sun* Air Viewer | Node ${this.data[0].address}` },
        { property: 'og:description',
          content: this.$i18n.locale === 'vi' ? `Biểu đồ chi tiết theo từng thời điểm mà sensor đo được tại ${this.data[0].address}`
            : `Detailed chart of each time the sensor is measured at ${this.data[0].address}` }
      ]
    }
  },
  computed: {
